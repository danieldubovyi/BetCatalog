...
{
    public record GetAccountsQuery(AccountFilter Filter) : IRequest<IReadOnlyCollection<AccountListItem>>;
    ...
    {
        public async Task<IReadOnlyCollection<AccountListItem>> Handle(GetAccountsQuery request, CancellationToken cancellationToken)
        {
            return await accountRepository.GetProjectedListAsync(request.Filter.GetExpression(),
